...
class CartController extends ControllerMVC {
  ...
  Future<void> refreshCarts() async {
    ...
    listenForCarts(message: AppLocalization.of(context).carts_refreshed_successfuly);
  }
  ...
  void removeFromCart(Cart _cart) async {
    ...
    removeCart(_cart).then((value) {
      calculateSubtotal();
      scaffoldKey?.currentState?.showSnackBar(SnackBar(
        content: Text(AppLocalization.of(context).the_product_was_removed_from_your_cart(_cart.product.name)),
      ));
    });
  }
  ...
  void goCheckout(BuildContext context) {
    if (!currentUser.value.profileCompleted()) {
      scaffoldKey?.currentState?.showSnackBar(SnackBar(
        content: Text(AppLocalization.of(context).completeYourProfileDetailsToContinue),
      ...
      if (carts[0].product.market.closed) {
        scaffoldKey?.currentState?.showSnackBar(SnackBar(
          content: Text(AppLocalization.of(context).this_market_is_closed_),
        ));
      } else {
