...
class ProductController extends ControllerMVC {
  ...
  void addToCart(Product product, {bool reset = false}) async {
    ...
    if (_oldCart != null) {
      ...
      updateCart(_oldCart).then((value) {
        ...
        scaffoldKey?.currentState?.showSnackBar(SnackBar(
          content: Text(AppLocalization.of(context).this_product_was_added_to_cart),
        ));
      });
    } else {
  ...
  void addToFavorite(Product product) async {
    ...
    addFavorite(_favorite).then((value) {
      ...
      scaffoldKey?.currentState?.showSnackBar(SnackBar(
        content: Text(AppLocalization.of(context).thisProductWasAddedToFavorite),
      ));
    });
  }
  ...
  void removeFromFavorite(Favorite _favorite) async {
    removeFavorite(_favorite).then((value) {
      ...
      scaffoldKey?.currentState?.showSnackBar(SnackBar(
        content: Text(AppLocalization.of(context).thisProductWasRemovedFromFavorites),
      ));
    });
  }
  ...
  Future<void> refreshProduct() async {
    ...
    listenForProduct(productId: _id, message: AppLocalization.of(context).productRefreshedSuccessfuly);
  }
