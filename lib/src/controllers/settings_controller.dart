...
class SettingsController extends ControllerMVC {
    ...
    _firebaseAuth.verifyPhoneNumber(
      ...
      verificationFailed: (authException) {
        ...
        scaffoldKey?.currentState?.showSnackBar(SnackBar(
          content: Text(AppLocalization.of(context).otpSendingFailed),
        ));
      },
      ...
      _firebaseAuth.signInWithCredential(_authCredential).then((authResult) async {
        ...
        if(authResult.user != null) {
          ...
          Scaffold.of(context).showSnackBar(SnackBar(
            content: Text(AppLocalization.of(context).failedToVerifyOTP),
          ));
        }
        ...
        if(error.toString().contains("ERROR_INVALID_VERIFICATION_CODE")) {
          ...
          Scaffold.of(context).showSnackBar(SnackBar(
            content: Text(AppLocalization.of(context).invalidOTPCode),
          ));
        }
        else {
  ...
  void update(User user) async {
    ...
    repository.update(user).then((value) {
      ...
      if(value != null && value.apiToken != null) {
        ...
        setState(() {});
        scaffoldKey?.currentState?.showSnackBar(SnackBar(
          content: Text(AppLocalization.of(context).profileUpdatedSuccessFully),
        ));
      }
      else {
        scaffoldKey?.currentState?.showSnackBar(SnackBar(
          content: Text(AppLocalization.of(context).failedToUpdateProfile),
        ));
      }
    });
  }
    ...
    repository.update(user).then((value) {
      ...
      if(value != null && value.apiToken != null) {
        ...
        try {
          ...
          scaffoldKey?.currentState?.showSnackBar(SnackBar(
            content: Text(AppLocalization.of(context).phone_change_success),
          ));
        } catch(error) {
          print(error);
        }
        ...
        scaffoldKey?.currentState?.showSnackBar(SnackBar(
          content: Text(AppLocalization.of(context).phone_change_failed),
        ));
      }
    });
  }
    ...
    repository.changePassword(password).then((value) {
      ...
      try {
        if(json.decode(value)['status']) {
          ...
          scaffoldKey?.currentState?.showSnackBar(SnackBar(
            content: Text(AppLocalization.of(context).password_change_success),
          ));
        }
        else {
          ...
          if(json.decode(value)['message'] == "Current password does not match") {
            ...
            scaffoldKey?.currentState?.showSnackBar(SnackBar(
              content: Text(AppLocalization.of(context).old_password_do_not_match),
            ));
          }
        }
      }
        ...
        print(error);
        scaffoldKey?.currentState?.showSnackBar(SnackBar(
          content: Text(AppLocalization.of(context).password_change_failed),
        ));
      }
    });
  }
  ...
  void updateCreditCard(CreditCard creditCard) {
    repository.setCreditCard(creditCard).then((value) {
      setState(() {});
      scaffoldKey?.currentState?.showSnackBar(SnackBar(
        content: Text(AppLocalization.of(context).payment_settings_updated_successfully),
      ));
    });
  }
