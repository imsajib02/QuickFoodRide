...
class TrackingController extends ControllerMVC {
  ...
  Future<void> refreshOrder() async {
    order = new Order();
    listenForOrder(message: AppLocalization.of(context).tracking_refreshed_successfuly);
  }
  ...
  void doCancelOrder() {
    cancelOrder(this.order).then((value) {
      ...
      listenForOrderStatus();
      scaffoldKey?.currentState?.showSnackBar(SnackBar(
        content: Text(AppLocalization.of(context).orderThisorderidHasBeenCanceled(this.order.id)),
      ));
    });
  }
