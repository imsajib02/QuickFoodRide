              ...
              return SimpleDialog(
                ...
                title: Row(
                  children: <Widget>[
                    ...
                    SizedBox(width: 10),
                    Text(
                      AppLocalization.of(context).payment_settings,
                    ...
                    child: Column(
                      children: <Widget>[
                        new TextFormField(
                          ...
                          decoration: getInputDecoration(hintText: '4242 4242 4242 4242', labelText: AppLocalization.of(context).number),
                          initialValue: widget.creditCard.number.isNotEmpty ? widget.creditCard.number : null,
                          validator: (input) => input.trim().length != 16 ? AppLocalization.of(context).not_a_valid_number : null,
                          onSaved: (input) => widget.creditCard.number = input,
                        ),
                        new TextFormField(
                            style: TextStyle(color: Theme.of(context).hintColor),
                            keyboardType: TextInputType.datetime,
                            decoration: getInputDecoration(hintText: 'mm/yy', labelText: AppLocalization.of(context).exp_date),
                            initialValue: widget.creditCard.expMonth.isNotEmpty ? widget.creditCard.expMonth + '/' + widget.creditCard.expYear : null,
                            // TODO validate date
                            validator: (input) => !input.contains('/') || input.length != 5 ? AppLocalization.of(context).not_a_valid_date : null,
                            onSaved: (input) {
                        ...
                        new TextFormField(
                          style: TextStyle(color: Theme.of(context).hintColor),
                          keyboardType: TextInputType.number,
                          decoration: getInputDecoration(hintText: '253', labelText: AppLocalization.of(context).cvc),
                          initialValue: widget.creditCard.cvc.isNotEmpty ? widget.creditCard.cvc : null,
                          validator: (input) => input.trim().length != 3 ? AppLocalization.of(context).not_a_valid_cvc : null,
